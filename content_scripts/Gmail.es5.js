...
InboxSDK.load('1', 'sdk_rupaparapankaj_32b6a78c77').then(function (sdk) {
    ...
    var commandTemplate = ['<CMD>', 'go to <CMD>', 'go to <CMD> mail', 'go to <CMD> email',
    //"go to <CMD> box",
    //"go to <CMD> folder",
    'show me <CMD>'];
    ...
    sdk.Compose.registerComposeViewHandler(function (composeView) {
        composeView.addButton({
            ...
            iconClass: 'btn-voice',
            onClick: function onClick(btnevent) {
                SwitchModeC_to_W();
            }
        });
    ...
    function SwitchModeC_to_W() {
        if (artyomWritten.isRecognizing()) {
            ...
            sdk.Compose.registerComposeViewHandler(function (composeView) {
                artyomWritten.redirectRecognizedTextOutput(function (recognized, isFinal) {
                    if (isFinal) {
                        $('.AO-widgets').hide(300);
    ...
    function SwitchModeW_to_C() {
        ResetWritingbutton();
        artyomWritten.fatality().then(function () {
            $('#btn-cmd').show(100).text('Command Mode');
            IsRuning = true;
    ...
    function OkayResponse(cmd) {
        ...
        artyomMain.sayRandom(["Okay", "Fine", "Sure", "All Right", "Cool"]);
    }
